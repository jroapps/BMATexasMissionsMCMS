...
const TrainingsPhotos: React.FC = () => {
    ...
    useEffect(() => {
        const photosRef = firestore.collection('TrainingPhotos');
    ...
    useEffect(() => {
        ...
        photosRef.get().then((snapshot) => {
            ...
            const photoId = "TPH" + (count + 1);
            setPhotoId(photoId);
        });
    }, []);

    // Function to view larger photo
    const viewPhoto = () => {
        console.log(photoId, pictureUrl, savePhoto(pictureUrl));
    }

    // Function to upload new photo
    const uploadPhoto = async () => {
        const photosRef = firestore.collection('TrainingPhotos').doc(photoId);
    ...
    async function savePhoto(blobUrl) {
        const pictureRef = storage.ref(`/TrainingPhotos/${photoId}`);
                ...
                <IonPage id="main">
                    <IonHeader>
                        <IonToolbar color="primary">
                            ...
                            <IonTitle>Trainings Photos</IonTitle>
                        </IonToolbar>
                    </IonHeader>
                            ...
                            <IonRow>
                                <IonCol>
                                    <IonList className="ion-padding">
                                        {first.map((firstHalf) =>
                                            <IonItem button key={firstHalf.id}
                                            onClick={viewPhoto}><br /><br />
                                ...
                                <IonCol>
                                    <IonList className="ion-padding">
                                        ...
                                        <IonItem button key={secondHalf.id} onClick={viewPhoto}
                                        ><br /><br />
...
export default TrainingsPhotos;